...
source("./R/RUNALL_extrapolate_20180907.R")
...
for (i in seq(1, length(names), by =2)) {
  ...
  fit_binary[[names[i]]] <- rpart(IVH_binary ~ . - IVH_range, data = IVH, method = "class")
  fit_numeric[[names[i+1]]] <- rpart(IVH_range ~ . - IVH_binary, data = IVH,  method = "anova")
