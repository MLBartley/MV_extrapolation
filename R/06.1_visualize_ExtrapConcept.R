...
## Updated 1: September 18, 2019 - cleaned up and saved with transparancies 
## 
###############################################################################
...
## data should already be log(data + 1) 
  ##data <- pull off just the TP and chla values
  ##data_cov <- pull off TP and chla values in addition to the covariates used
...
## plot and shade areas of extrapolation
...
#where is the cutoff?
where <- which.max(diag(hat))
shade <- abs(X[where])

#     ## add the shading and prediction interval/CI
...
dd + geom_point(size = 4) + 
  theme(text = element_text(size=50))

ggsave("./figures/figure1.eps", 
       device=cairo_ps, #ensures the shading stays
