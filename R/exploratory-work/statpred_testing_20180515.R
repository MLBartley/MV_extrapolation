##Messing around with SpatPred package
## May 15 2018
## Ultimate goal is to extend methods to MV case
...
Cur.grid@data = cbind(Cur.grid@data,
                      ice_conc2,
                      dist_edge2,
                      dist_shelf2,
                      dist_mainland2)
...
Which.no.ice = Which.no.ice[which((Which.no.ice %in% Which.not.sampled) ==
                                    TRUE)]
...
Control = list(
  ...
  adapt = TRUE,
  srr.tol = NULL,
  fix.tau.epsilon = FALSE,
  predict = TRUE,
  MH.nu = rep(0.5, length(Mapping))
)
Data = Cur.grid
spat.mod = 0
Offset = Area.photo
...
glm.out <-
  spat_pred(
    ...
    Offset = Offset,
    Effort = Effort,
    Area.adjust = Area.adjust,
    Control = Control,
    Assoc = Assoc,
    Knots = Knots,
    Prior.pars = Prior.pars
  )
...
cell.width = 25067
naive.glm.plot = plot_N_map(
  1,
  matrix(apply(glm.out.cov$MCMC$Pred, 1, 'mean'), S, 1),
  Grid = Grid.list,
  leg.title = "Abundance",
  highlight = which(glm.out.cov$gIVH == 0),
  cell.width = 25067,
  hcolor = 'black'
)
...
naive.rsr.plot = plot_N_map(
  1,
  matrix(apply(rsr.out$MCMC$Pred, 1, 'mean'), S, 1),
  Grid = Grid.list,
  leg.title = "Abundance",
  highlight = which(rsr.out$gIVH == 0),
  cell.width = 25067,
  hcolor = 'black'
)
...
ice0.glm.plot = plot_N_map(
  1,
  matrix(apply(glm.out.ice0$MCMC$Pred, 1, 'mean'), S, 1),
  Grid = Grid.list,
  leg.title = "Abundance",
  highlight = which(glm.out.ice0$gIVH == 0),
  cell.width = 25067,
  hcolor = 'black'
)
...
rsr.ice0.plot = plot_N_map(
  1,
  matrix(rowMeans(rsr.ice0.out$MCMC$Pred), S, 1),
  Grid = Grid.list,
  leg.title = "Abundance",
  cell.width = 25067,
  hcolor = 'white'
...
xy <- laply(Cur.grid@polygons, function(x)
  x@Polygons[[1]]@labpt)
rib$x <- xy[, 1]
rib$y <- xy[, 2]
...
cat(paste(
  "quasipoisson abundance w/o pseudo-absences = ",
  sum(N.naive.qp),
  "\n"
))
...
# delta method
breadpred <-
  pred2$pred.area * (c(dtmfn(lpmat.pred %*% coef(b))) * lpmat.pred)
varpred <- breadpred %*% (vcov(b) %*% t(breadpred))

lpmat.pred <- predict(b.naive, newdata = pred, type = "lpmatrix")
# delta method
breadpred <-
  pred$pred.area * (c(dtmfn(lpmat.pred %*% coef(b.naive))) * lpmat.pred)
...
ice0.gam.plot = plot_N_map(
  1,
  matrix(rib2$N, ncol = 1),
  Grid = Grid.list,
  leg.title = "Abundance",
  cell.width = 25067,
  highlight = which(rib2$out == 1),
  hcolor = 'black'
)
naive.gam.plot = plot_N_map(
  1,
  matrix(rib$N, ncol = 1),
  Grid = Grid.list,
  leg.title = "Abundance",
  cell.width = 25067,
  highlight = which(rib$out == 1),
  hcolor = 'black'
)
...
naive.glm.plot = naive.glm.plot + ggtitle("A. GLM (Naive)") + scale_fill_gradientn(
  limits = c(0, 6000),
  colours = myPalette(100),
  breaks = c(0, 2000, 4000, 6000)
) + theme(plot.title = element_text(hjust = 0, size = rel(1)))
ice0.glm.plot = ice0.glm.plot + ggtitle("B. GLM (Pseudo-absence)") + scale_fill_gradientn(
  limits = c(0, 6000),
  colours = myPalette(100),
  breaks = c(0, 2000, 4000, 6000)
) + theme(plot.title = element_text(hjust = 0, size = rel(1)))

naive.gam.plot = naive.gam.plot + ggtitle("C. GAM (Naive)") + scale_fill_gradientn(
  limits = c(0, 6000),
  colours = myPalette(100),
  breaks = c(0, 2000, 4000, 6000)
) + theme(plot.title = element_text(hjust = 0, size = rel(1)))

ice0.gam.plot = ice0.gam.plot + ggtitle("D. GAM (Pseudo-absence)") + scale_fill_gradientn(
  limits = c(0, 6000),
  colours = myPalette(100),
  breaks = c(0, 2000, 4000, 6000)
) + theme(plot.title = element_text(hjust = 0, size = rel(1)))

naive.rsr.plot = naive.rsr.plot + ggtitle("E. STRM (Naive)") + theme(plot.title = element_text(hjust = 0, size =
                                                                                                 rel(1))) + scale_fill_gradientn(colours = myPalette(100))

rsr.ice0.plot = rsr.ice0.plot + ggtitle("F. STRM (Pseudo-absence)") + scale_fill_gradientn(
  limits = c(0, 6000),
  colours = myPalette(100),
  breaks = c(0, 2000, 4000, 6000)
) + theme(plot.title = element_text(hjust = 0, size = rel(1)))
